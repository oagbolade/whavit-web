              ...
              <div class="job-overview">
                <div class="job-overview-inner">
                  <!-- <router-link :to="'/user/dashboard/request/assign/'+requestId" v-if="request.vendor < 1">
                      <button class="btn mx-2 btn-primary btn-md">Assign a vendor</button>
                  </router-link> -->
                  ...
                  <router-link :to="'/user/dashboard/request/reschedule/'+requestId" v-if="request.paid">
                    <button class="btn mx-2 btn-primary btn-md">Reschedule Cleaning</button>
                  </router-link>
                  <router-link :to="'/user/dashboard/request/tasks/'+requestId" v-if="request.paid && currentDate-timestamp >= 0 && request.status == 'accepted'" >
                    <button class="btn mx-2 btn-success btn-md">Review Cleaning</button>
                  </router-link>

                  <router-link :to="'/user/dashboard/request/payment/'+requestId" v-if="!request.paid">
                    <button class="btn mx-2 btn-primary btn-md">Pay For Your Cleaning Schedule </button>
                  </router-link>

                    <button class="btn text-white btn-success btn-sm float-right small" v-if="request.status == 'accepted'">
                      WhavPro Is Ready
                    </button>
                    <button class="btn text-white btn-warning btn-sm float-right small" v-if="request.status == 'pending' && request.vendor >= 1">
                      WhavPro Yet To Accept
                    </button>
                    <button class="btn text-white btn-warning btn-sm float-right small" v-if="request.status == 'pending' && request.vendor < 1 && !request.paid">
                      Payment Not Made Yet
                    </button>
                    <button class="btn text-white btn-danger  btn-sm float-rightsmall" v-if="request.status == 'rejected'">
                      Vendor Rejected
                    </button>
                    <button class="btn text-white btn-danger  btn-sm float-rightsmall" v-if="request.status == 'cancelled'">
                      Cancelled
                    </button>
              ...
              <div class="single-page-section">
                <div class="job-overview mt-2">
                  <div class="job-overview-headline">
                    <span v-if="request.products.length>0 && request.products[0].name">
                  ...
                  <div class="job-overview-inner">
                      <div class="row mt-3">
                        <!-- <div class="col-md-4" v-if="request.products.length > 0 && request.products[0].name"> -->
                        <div class="col-md-4">
              ...
              <div class="sidebar-container">
                ...
                <ul class="list-group">
                  ...
                  <!-- <li class="list-group-item px-0 bg-none border-0" v-if="request.vendor < 1">
                    <router-link :to="'/user/dashboard/request/assign/'+requestId">
        ...
        if (result.value) {
          ...
          this.$http({
            ...
            .then(res => {
              ...
              this.$router.push("/user/dashboard/requests");
            })
            .catch(err => {
              console.log(err);
              loader.hide();
...
mounted() {
    ...
    this.$http({
      ...
      .then(res => {
        ...
        let tempDate = res.data.data.start_date;
        let date = Date.parse(tempDate.split('/').reverse().join('-'));
        let tempCurrentDate = new Date().toISOString().slice(0,10);
        let currentDate = Date.parse(tempCurrentDate.split('/').reverse().join('-'));
        this.timestamp = date;
