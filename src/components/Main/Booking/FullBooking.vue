                            ...
                            <div class="col-lg-12 form-inline">
                              ...
                              <div class="col-lg-4">
                                <div class="form-group">
                                  <label for="no_of_rooms" class="text-muted"
                              ...
                              <div class="col-lg-4">
                                <div class="form-group">
                                  <label for="schedule" class="text-muted"
                                  ...
                                  >
                                    <option value="once"
                                      >One Time cleaning</option
                                    >
                                    <option value="weekly"
                                      >Weekly cleaning</option
                                    >
                                    <option value="bi-weekly"
                                      >Bi-Weekly Cleaning
                                    </option>
                                    <option value="monthly"
                                      >Monthly Cleaning</option
                                    >
                          ...
                          <div class="row">
                            <div class="col-lg-12">
                              <b-alert show variant="primary"
                                >This service is optional</b-alert
                              >
                              <h4 class="text-muted">
                            ...
                            <div class="col-lg-12">
                              ...
                              <div class="switches-list">
                                ...
                                >
                                  <div
                                    v-if="
                                      checkMattressUpholstery(service.title)
                                    "
                                  >
                                    <label class="switch">
                                      <input
                                    ...
                                    <!-- <b-modal :id="'cards'+service.id" :title="'Let us get more information to provide an excellent '+service.title +' service'" v-if="service.attribute_name.length >= 1 && service.clicked === 'clicked'+service.id" no-enforce-focus>
...
export default {
  ...
  methods: {
    checkMattressUpholstery(title) {
      if (title === "Mattress Hygiene" || title === "Upholstery Cleaning") {
        return true;
      }
      return false;
    },
    ...
    toggleChecked(id) {
      let addAllPrices = 0;
      //this.tempPriceTracker = addAllPrices;

      let filtered_array = this.services.filter(service => {
        return id === service.id;
      });

      if (filtered_array.length > 0) {
        filtered_array.forEach(attribute_name => {
          attribute_name.attribute_name.forEach(attribute => {
            attribute.attribute.forEach(getDetails => {
              addAllPrices +=
                getDetails.price * this.otherServicesInput[getDetails.id];
            });
          });
        });
      }
      ...
      if (this.currentlyChecked.includes(id)) {
        ...
        // We remove the pricing at this point
        return false;
      ...
      this.$http({
        ...
        .then(res => {
          ...
          this.products.forEach(product => {
            if (product.name.indexOf(names.light) != -1) {
              this.arrangedProducts[0] = product;
            }

            if (product.name.indexOf(names.deep) != -1) {
              this.arrangedProducts[1] = product;
            }

            if (!this.arrangedProducts.includes(product, 2)) {
              this.arrangedProducts.push(product);
            }
          });
          this.arrangedProducts = [...new Set(this.arrangedProducts)];
          // loader.hide();
        })
        .catch(err => {
          // loader.hide();
        ...
        .then(res => {
          ...
          this.services.forEach(service => {
            if (service.attribute_name.length > 0) {
              service.attribute_name.forEach(attribute => {
                if (attribute.attribute.length > 0) {
                  attribute.attribute.forEach(attributes => {
                    this.inputIds.push(attributes.id);
                  });
                }
              });
            }
    ...
    checkMeta(attr, meta_amount, service_id) {
      // Set this to retain input fields after clicking next
      this.otherServicesInput[attr.id] = meta_amount;
      //console.log("Logged Output: checkMeta -> this.otherServicesInput", this.otherServicesInput)
    ...
    paymentFirstStep(bookingData) {
      ...
      if (this.payment_method == "wallet") {
        if (this.wallet.balance < this.price) {
          this.$swal.fire({
            text: "Insufficient wallet balance, try paying with card",
            type: "error"
          });
          loader.hide();
        } else {
    ...
    createBooking(booking) {
      ...
      this.$http({
        ...
        .then(res => {
          ...
          console.log("Logged Output: createBooking -> bookingData", booking);
          window.dataLayer = window.dataLayer || [];
          window.dataLayer.push({
            event: "fullCleaningServiceSucessfullyBooked",
            ...
            productQuantity: 4, //return number of services requested for by the customer
            serviceCategory: "Full Cleaning Service", //leave this like this
            cleaningType: "light cleaning", //return with the selected product, light or deep or post construction cleaning
            numberOfBedroom: bookingData.number_of_rooms, //replace with bedroom requested to be cleaned by the customer
            bookingschedule: "one time cleaning", //return with the cleaning schedule, which can be one time, weekly, bi weekly, monthly or bi monthly cleaning.
            extraCleaningSelected: [
              "Inside of Refrigerator",
              "Inside of Cabinet",
              "Inside of Oven"
            ], //return in an array format, selected extra cleaning if any exist
            specialServicesSelected: [
              "Mattress Hygiene",
              "Upholstery Cleaning"
            ], //return successful if it is, and if it failed return failed
            timeSelected: booking.time, //return the hour for the scheduled cleaning, leaving the value in double digit
            timeCategory: null, //return the time category for the scheduled cleaning with either pm or am
            dateSelected: booking.start_date, //return the date selected in the calendar by te customer
            daySelected: null, //return day, do this by extracting the day from the date selected
            cleaningLocation: bookingData.location, //return the location selected by the user while requesting for cleaning
            cleaningAddress: bookingData.address, //return the full address of the user, inputed in the address bar
            accountType: this.user.type //replace with type of account and leave blank where not present
          });
          //Add Discount Here
          if (this.discount_code !== null) {
